...
class ModelBase(object):
    ...
    '''
        ...
        self.config = config
        self.logger = Logger(logdir=config.logdir, flush_secs=5)
        if _logger is None:
            self.print_fn = print
        else:
    ...
    def build_optimizer(self):
        ...
        if len(trn_params) > 0:
            optimizer = optim.Adam(per_param_opts, lr=self.config.base_lr, weight_decay=2e-5)
    ...
    def pretty_print_metrics(self, prefix, metrics):
        ...
        for measure, score in metrics.items():
            if (measure is not 'loss'):
                self.logger.log_value(measure, score, step=ste)
            else:
                self.logger.log_value('epoch_avg_loss', score, step=ste)
    ...
    def train_one_batch(self, batch_data, step):
        ...
        self.logger.log_value('batch_loss', loss_value, step=step)
